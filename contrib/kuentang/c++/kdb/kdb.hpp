...
# ifndef KDB_HPP_KKT_11_09_2011
# define KDB_HPP_KKT_11_09_2011
...
# define kdb_ktc_symbol(x) return std::string(x);
...
# define kdb_ctk_symbol(x) return ss(x.c_str());
...
((symbol				,kx_KS,s,S,kx_kS,S*,	kdb_ktc_symbol,std::string						,kdb_ctk_symbol			))									\
...
namespace qtype{
	...
	boost::gregorian::date const gday = boost::gregorian::date(2000,boost::gregorian::Jan,1);
	boost::gregorian::date const bday = boost::gregorian::date(2000,boost::gregorian::Jan,1);
	boost::posix_time::ptime const pday = boost::posix_time::ptime(gday,boost::posix_time::time_duration(0,0,0,0));
