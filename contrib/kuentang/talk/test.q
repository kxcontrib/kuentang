...
matur:1.0;spot:15.0;
/ arg:`type_`spot`drift`diffu`matur`steps`repl`output!
/    (`euler;(spot;theta);{[t;s] ((mu*s[0]),kappa* theta - s[1]) };{[t;s] (s[0],1.0)*W};matur;steps;num;`wtime);
arg:`type_`spot`matur`steps`repl`output!(`euler;(spot;theta);matur;steps;num;`wtime)
arg,:`drift`diffu!({[t;s] ((mu*s[0]),kappa* theta - s[1]) };{[t;s] (s[0],1.0)*W})
path:.ql.paths arg;
time:path 0;path:path 1;
B:first flip path;lspr:last flip path;A:B*exp lspr;spr:B-A;
sellpoint:theta+0.5*eta;buypoint:theta-0.5*eta;
/([] time;spr 0; sellpoint;buypoint)
decise:()!();
decise[0f]:{[spread] :$[spread>sellpoint;-1f; $[spread<buypoint;1f;0f] ] };
decise[1f]:{[spread] :$[spread>theta;0f; 1f] };
decise[-1f]:{[spread] :$[spread<theta;0f;-1f] };

dec:flip {decise[x] {x y}' y}\[count[lspr]#0f;flip lspr]
pos:flip deltas flip dec;
cfs:pos*spr
{sum [cfs x] -last[spr x ]*sum pos x } 2

reports1:([]sum flip pos;last flip spr;cfs:sum[flip cfs];pnl:sum[flip cfs]-(last flip spr)*sum flip pos)

select c:count i by 1 xbar pnl from reports1
